...
AckermannSteering::AckermannSteering()
{
    ...
    breakPub = node.advertise<std_msgs::Float32>("vrep/break",10);
}
...
void AckermannSteering::process()
{
    ...
    if(joyin != NULL)
    {
      if(joyin->buttons[20]==1){
        ...
        breakSignal.data = joyin->axes[2];
        breakPub.publish(breakSignal);
        ...
        //Control the break and drive
        if (mode == 1)      //drive forward
        {
           if(joyin->axes[1] < 0)
           {
                drive.data = (joyin->axes[1] * 10 +10) - (joyin->axes[2] * 10 +10);
           }
           else
           {
                drive.data = (joyin->axes[1] * 10 +10) - (joyin->axes[2] * 10 +10);
        ...
        if(mode==2)     //reverse
        {
           if(joyin->axes[1] < 0)
           {
                drive.data = ((joyin->axes[1] * 10 +10)*(-1)) - ((joyin->axes[2] * 10 +10)*(-1));
           }
           else
           {
                drive.data = ((joyin->axes[1] * 10 +10)*(-1)) - ((joyin->axes[2] * 10 +10)*(-1));
